...
```bash
poetry install
poetry run python -m insurance_calc
```
...
```bash
INSURANCE_CALC_RELOAD="True"
INSURANCE_CALC_PORT="8000"
INSURANCE_CALC_ENVIRONMENT="dev"
```
...
```
docker run -p "5432:5432" -e "POSTGRES_PASSWORD=insurance_calc" -e "POSTGRES_USER=insurance_calc" -e "POSTGRES_DB=insurance_calc" postgres:16.3-bullseye
```
